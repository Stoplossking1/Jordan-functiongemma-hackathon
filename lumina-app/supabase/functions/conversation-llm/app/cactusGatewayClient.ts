...
const TRANSCRIBE_TIMEOUT_IN_MS = 30_000;
...
export class CactusGatewayClient {
  ...
  async transcribe(audioBase64: string, audioMimeType: string, prompt?: string): Promise<TranscribeResult> {
    const abortController = new AbortController();
    const timeoutId = setTimeout(() => abortController.abort(), TRANSCRIBE_TIMEOUT_IN_MS);
    ...
    try {
      ...
      try {
        ...
        if (abortController.signal.aborted) {
          throw new Error(`Cactus transcribe timeout after ${TRANSCRIBE_TIMEOUT_IN_MS}ms`);
        }
        throw error;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(`Cactus transcribe failed with status ${response.status}: ${errorText}`);
      }
      ...
      return {
        ...
        totalTimeInMs: responseJson.totalTimeInMs ?? 0,
      };
    } finally {
      clearTimeout(timeoutId);
    }
